...
- [ ] `[UNIVERSAL]` **`snake_case` verb_noun names** — e.g. `list_dialogs`, `get_entity_info`, `submit_feedback`. No `getData`, `RunQuery`, `handle_request` (too generic), no spaces or special chars. Characters: `^[a-zA-Z0-9_-]{1,64}$` — cross-client safe pattern (snake_case base with hyphens for namespacing); see [tool-design.md §Character set](tool-design.md) and [clients.md](clients.md) for spec vs. client-enforcement details.
