...
The statusline has a `vibes` block that tells you how your API spending is going relative to your 5-day budget window — without making you do math.

During the week you get a **pace label** and a **delta** (how far ahead or behind the expected burn rate you are):

| Label | What it means |
|-------|---------------|
| `depresso` | Barely spending anything — are you even coding? |
| `salty` | Slightly under pace |
| `chill` | Right on track, comfortable margin |
| `hyped` | Running a bit hot |
| `based` | Going fast, burning bright |
